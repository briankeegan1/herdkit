...
| `review_wake_result` | slug, pane, woke, retried(none\|enter\|prompt), pane_closed | `driver.sh:herd_review_wake_verify` (HERD-523/#633 — the `spawn_wake_result` check applied to REVIEW dispatch; `pane_closed` records whether the stalled reviewer pane was actually torn down or the guarded close REFUSED it) |
| `status_disagreement` | slug, pane, agent_status(idle), resolved(working) | `driver.sh:herd_driver_agent_status_resolved` (HERD-647/#632-fallout — a raw `idle` roster read was overridden after BOTH the pane's active-chrome signal and a genuine content delta corroborated it; the sole trigger for this event, so its presence is the complete rescue record) |
| `review_viewer_tab_closed` | slug, tab, reason(verdict-consumed) | `agent-watch.sh:_close_review_viewer_tab` (HERD-523/#634 — the standalone `review·<slug>` viewer tab retired by SLUG-LABELED lookup on verdict consumption, so teardown never depends on the `.herd-tabs` registry row having landed) |
| `tab_registry_write_failed` | component, slug, tab, registry, kind | `herd-review.sh` (HERD-523/#634 — the `.herd-tabs` allowlist append failed, so the sweep cannot see this tab; replaces a silent `\|\| true`) |
...
| `stale_base_autofix_bounce` | pr, sha, slug, reason | port `live_runtime.py:LiveTick._stale_no_wake_fallback` (HERD-584 — the stale-base rail's `_bounce_and_wake` `no_wake_fallback`: nobody is on the live pane to re-task with the merge-up, so the existing conflict resolver, `herd-resolve.sh`, is dispatched in its place instead of escalating a MECHANICAL fix to a human. Still spends the rail's round like a real bounce — a resolver dispatch is a real heal attempt, unlike an unwoken bounce, which refunds) |
...
| `scope_unresolved` | gate(watcher_scope), detail | port `live_runtime.py:LiveTick._check_scope_unresolved` (HERD-653, GH #769 — the WATCHER_SCOPE ownership gate, task HERD-324 leg 3, never reached the python core's environment at all: FAIL-CLOSED rather than the pre-fix silent degrade to the permissive `mine` default. `_select_candidates` withholds every candidate not provably the resolved operator's own for the rest of that tick. Journaled once per signature — `LiveState.scope_unresolved_once`/`clear_scope_unresolved` — so an ongoing wiring gap is loud exactly once and a LATER regression, after a clean recovery, is loud again) |
...
| `ci_health_verdict` | pr, sha, slug, verdict(CLEAN\|CODEERROR\|WAIT), reason, detail, workflow, conclusion, run_id, cancelled | port `live_runtime.py:LiveGates._ci_health` (HERD-578 — the health verdict READ from the PR's own CI under `HEALTH_SOURCE=ci`, mapped by the shared `herd.ci_verdict`. `reason` ∈ `ci-green` \| `ci-code-red` \| `ci-infra-transient` \| `ci-log-unreadable` \| `ci-pending` \| `ci-cancelled-chain` \| `ci-no-run-yet`. Journaled once per (pr, sha, verdict, reason): the read itself is per-tick, the EVENT is a phase change) |
| `ci_health_fallback` | pr, sha, slug, reason(ci-absent\|gh-unavailable\|no-branch), detail | port `live_runtime.py:LiveGates._ci_fallback` (HERD-578 — `HEALTH_SOURCE=ci` could not read CI at all, so the LOCAL suite is carrying the gate after all. Loud by contract: never a silent skip) |
...
| `ci_verdict_healed` | pr, sha, slug, reason(rerun), workflow, conclusion, run_id | port `live_runtime.py:LiveGates.ci_verdict_recheck` (HERD-677 — a CI-sourced CODEERROR memo (HERD-649b) is re-verified against live CI on a bounded cadence, throttled by `_ci_codeerror_recheck_due`; a rerun that flipped the SAME sha green journals this and returns CLEAN in its place, so the ordinary `refix_rail_reset` refunds the rail and the PR becomes merge-eligible with no further bounce) |
