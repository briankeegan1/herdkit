...
| `REVIEW_AUTOFIX` / `REFIX_MAX_ROUNDS` | Auto-bounce a BLOCK review back to the builder to fix + push, bounded by max rounds (default 3) before escalating to "needs you". |
| `STALE_BASE_AUTOFIX` | Auto-heal STALE-BASE holds from the pre-merge stale-duplicate gate (default off): bounce the live builder with `git merge $DEFAULT_BRANCH` (or dispatch the conflict resolver when no live builder remains), sharing the `REFIX_MAX_ROUNDS` budget; DUPLICATE flavor stays human. |
...
| `HEALTHCHECK_AUTOFIX` | Auto-bounce a reproduced healthcheck **code error** back to the builder (failing test + suite-log path), on the same rails as `REVIEW_AUTOFIX` and sharing ONE per-PR `REFIX_MAX_ROUNDS` budget with it. Limit-parked / dead agents are preflighted; a `tab-leak-guard` trip is infra and never bounced. Default `false`. A row only reads "needs you" when nobody is working that red. |
