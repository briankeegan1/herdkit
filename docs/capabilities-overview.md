...
| `CI_AUTOREPAIR` | Auto-heal **inherited** GitHub CI reds (default off): when a PR is MERGEABLE+UNSTABLE with a failing required check, `herd/gates` already passed, and the branch is behind main, base-refresh (merge `$DEFAULT_BRANCH`) on the `ci` refix rail — never silent-merges a real new-code CI failure. |
...
| `WATCH_HOTKEYS` | **In-console hotkey (opt-in, default `off`).** `on` arms a bounded keypress listener on the watcher console and adds a `v: view (<lens>)` hint row; pressing `v` flips `WATCHER_VIEW` `mine` ↔ `all` **in memory, for that one watcher process** (from any other lens — `all`, `review-queue`, `deps` — the first press lands on `mine`). It never writes `.herd/config` or `.herd/config.local`: `herd config set` remains the durable path, and a watcher restart (or a second seat) reverts to whatever the file says. Armed **only** when the watcher's own stdin is a real interactive tty — headless, sim, lib-mode, or a piped/redirected stdin degrade silently (no listener, no row, no error). Non-blocking by construction: the one blocking read lives in a single background listener spawned once at startup, and the tick loop only ever reads the small marker file that listener writes. `off` is byte-inert — no `stty` call, no stdin read, no listener process, no hint row. Machine-scoped (routes to `.herd/config.local`). |
