...
| **P3.4 DONE (HERD-425).** The engine moved to the Python live runtime (`pysrc/herd/live_runtime.py`) since this row was written, so the three legs are real SIGKILL injection seams at the CURRENT production boundaries — `LiveActuator.merge` (mid_do_merge), `LiveGates.health` (mid_gate_collect), `LiveTick._bounce_and_wake` (mid_refix_bounce) — gated by `HERD_CHAOS_KILL_AT`/`HERD_CHAOS_GUARD` (ship-dormant, byte-identical-when-off, fail-closed). `mid_do_merge`'s recovery path is `_sweep_merged_prs` (still bash, still the shared reconciler for every Python-actuated merge, not only crashed ones — `postmerge-reconcile-sim.sh` already proves that reconciler's own convergence; this leg proves a real Python SIGKILL lands in the exact shape it heals). Sim: `tests/test-gate-reconciler-chaos-sim.sh`. Unit seam-guard + restart-reconciliation coverage: `tests/test_live_runtime.py::TestChaosSeamGuard`/`TestChaosRestartReconciliation`. Registered as `proof_kind=sim` for `agent-watch.sh`, `HEALTH_CONCURRENCY`, `REFIX_MAX_ROUNDS` in `templates/conformance.tsv`. | | |
