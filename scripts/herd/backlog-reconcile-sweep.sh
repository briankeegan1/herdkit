...
# against recently-MERGED PRs (`gh pr list --state merged`) and recent commit subjects (`git log`)
# and surfaces the 🔜 items whose key terms STRONGLY match merged work — i.e. probably-shipped-but-
# still-🔜. It is deliberately:
...
#   HERD_SWEEP_PRS_FILE      file of "<pr#>\t<title>" lines to use instead of `gh pr list`.
...
# ── matcher (pure python; the tests drive it through the two file seams) ─────
# For each OPEN 🔜 item it extracts the distinctive key terms of the item TITLE (lowercased alnum
# tokens, ≥3 chars, not pure digits, minus a stop/domain-generic list), then finds the merged
# PR / commit whose subject shares the most of those terms. Confidence = matched/total terms.
...
# When EMIT_TSV is set, HIGH candidates are ALSO written there as
#   <lineno>\t<title>\t<kind>\t<ref>\t<subject>\t<score>\t<matched>/<total>
# for the --enqueue path to consume — matching runs exactly once.
_match() {
  BACKLOG="$BACKLOG" \
  PRS="$(_merged_prs)" COMMITS="$(_recent_commits)" \
...
def open_items(path):
    ...
    for i, ln in enumerate(lines, start=1):
        ...
        items.append((i, title, set(terms(title))))
    return items
...
for (lineno, title, iterms) in items:
...
if not rows:
    ...
    if n_hi:
        print("Run with --enqueue to file a scribe reconcile request for the %d HIGH-confidence "
              "candidate(s) (the scribe verifies before touching %s)." % (n_hi, os.path.basename(backlog)))
    else:
        print("No HIGH-confidence candidates — nothing would be enqueued. Review the MED matches by hand.")

# Machine-readable HIGH rows for the --enqueue path.
if emit_tsv:
    with open(emit_tsv, "w", encoding="utf-8") as f:
        for (conf, score, n, total, lineno, title, kind, ref, subj, matched) in rows:
            if conf != "HIGH": continue
            f.write("%d\t%s\t%s\t%s\t%s\t%.2f\t%d/%d\n" %
                    (lineno, title, kind, ref, subj, score, n, total))
'
}
...
_build_request() {
  local title="$1" kind="$2" ref="$3" subj="$4" score="$5" matched="$6" lineno="$7" refname verify
...
if [ "$ENQUEUE" -eq 0 ]; then
  _match                                       # report-only (default): print ranked candidates
  exit 0
fi
...
EMIT_TSV="$TSV" _match
n=0
while IFS=$'\t' read -r lineno title kind ref subj score matched; do
  [ -n "${lineno:-}" ] || continue
  req="$(_build_request "$title" "$kind" "$ref" "$subj" "$score" "$matched" "$lineno")"
