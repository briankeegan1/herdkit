...
# RESOLVED runtime driver so each runtime gets the mechanism it actually has:
#   • a runtime with a by-name selector (DRIVER_AGENT_SELECT_FLAG) gets the flag appended to the
#     spawn's <flags>, which herd_driver_agent_spawn_argv already word-splits into the argv — so
#     selection needs NO new seam in the spawn composer;
#   • a runtime with none gets the definition body injected into the task spec ($AGENT_SPEC_BLOCK,
#     composed below with the rest of the prompt).
# Exactly one of the two is ever non-empty, and BOTH are empty when HERD_AGENT is unset — so the
# default spawn's argv and task spec are BYTE-IDENTICAL to before this existed.
# A name whose last verify said it does NOT resolve prints a LOUD warning and spawns anyway: the
# operator's build is never held hostage to a probe, but they never discover the silent-fallback trap
# by watching the builder make the mistakes its definition forbids.
_AGENT_SELECT=""
...
if [ "$_HERD_DRIVER_NAME" = "headless" ]; then
  echo "🐑 Sub-agent '$SLUG' running detached ($_AGENT_ARGV_DESC)   dir: $DIR"
  ...
  echo "🐑 Sub-agent '$SLUG' running ($_AGENT_ARGV_DESC) in herdr tab $TAB   dir: $DIR"
  echo "   task spec: $TASK_SPEC_FILE   (builder got a short pointer to it, not the full spec inline)"
