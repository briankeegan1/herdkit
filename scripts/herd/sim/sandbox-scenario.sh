...
while [ $# -gt 0 ]; do
  case "$1" in
    --artifacts) [ $# -ge 2 ] || { echo "sandbox-scenario: --artifacts requires a value" >&2; exit 1; }; ART="$2"; KEEP=1; shift 2 ;;
    --keep)      KEEP=1; shift ;;
    --posture)   [ $# -ge 2 ] || { echo "sandbox-scenario: --posture requires a value" >&2; exit 1; }; POSTURE="$2"; shift 2 ;;
