...
#   HERD_REVIEW_SHA                (HERD-230) dispatch/pin sha — the commit the verdict is keyed to;
#                                  when set (and objects resolve under $MAIN), the reviewer reads
#                                  `git diff <merge-base>..<sha>` from $MAIN instead of live `gh pr diff`
#   HERD_REVIEW_PIN_MODE           (HERD-230) `pinned` | `unpinned` from the watcher's pin helper;
#                                  informational — missing objects always fall back to live `gh pr diff`
#
# Standalone:
...
# (HERD-230) is always read from $MAIN via `git -C $MAIN diff …`; CWD only supplies repo context.
...
# _emit_verdict <line> [rubric-lines] — rubric-lines (HERD-400), when non-empty, is written to the
# result FILE ahead of the verdict line — the stdout contract stays exactly one line, unchanged; only
# the file (which the python collector reads with parse_review_verdict, tolerant of extra lines)
# gains the criterion rows. Omitted/empty (the RUBRIC_FILE-unset default) → the file stays the single
# verdict line it always was, byte-identical.
_emit_verdict() {
  printf '%s\n' "$1" 2>/dev/null || true
...
# RUBRIC_FILE (HERD-400, docs/rubric-primitive.md): a structured per-unit review rubric — TSV rows
# `id<TAB>text<TAB>weight<TAB>pass_condition`, weight ∈ required|advisory, `#`-comments and blank
# lines skipped. Resolved with the SAME worktree-then-main lookup as REVIEW_CHECKLIST just above.
# Absent, or naming a file with zero well-formed rows, leaves both blocks empty — every prompt below
# is byte-identical to before this key existed. Two renderings share the same criteria list: RUBRIC_BLOCK
# (print-to-stdout wording, for LOCAL_TASK/TASK/PR_PANEL_TASK) and RUBRIC_BLOCK_AGENT (write-to-
# RESULT_FILE wording, for AGENT_TASK) — the reviewer's SINGLE REVIEW: verdict line is, in both, still
# the ONLY thing the merge gate reads; the rubric only ever informs that reviewer's own judgement.
RUBRIC_BLOCK="" RUBRIC_BLOCK_AGENT=""
_rubric_path=""
[ -n "$RUBRIC_FILE" ] && { [ -f "$CWD/$RUBRIC_FILE" ] && _rubric_path="$CWD/$RUBRIC_FILE"; [ -f "$MAIN/$RUBRIC_FILE" ] && _rubric_path="$MAIN/$RUBRIC_FILE"; }
if [ -n "$_rubric_path" ]; then
  _rubric_lines="" _rubric_ids=""
  while IFS=$'\t' read -r _rc_id _rc_text _rc_weight _rc_cond || [ -n "$_rc_id" ]; do
    case "$_rc_id" in ''|'#'*|id) continue ;; esac
    [ -n "$_rc_text" ] || continue
    _rc_weight="${_rc_weight:-advisory}"
    _rubric_lines="${_rubric_lines}  - [$_rc_id] ($_rc_weight) $_rc_text — pass: $_rc_cond"$'\n'
    _rubric_ids="${_rubric_ids:+$_rubric_ids, }$_rc_id"
  done < "$_rubric_path"
  if [ -n "$_rubric_ids" ]; then
    _rubric_header="Also judge the REVIEW RUBRIC for this project — one line per criterion:"$'\n'"${_rubric_lines}"
    RUBRIC_BLOCK="${_rubric_header}For EACH criterion above (in this order: ${_rubric_ids}), print one line, BEFORE your final REVIEW: line and nothing else on that line: 'RUBRIC: <id> | PASS|FAIL | <one-line reason>'. These lines inform your reasoning; they do NOT by themselves decide PASS/BLOCK — your final REVIEW: line is still the ONLY thing the merge gate reads to decide the merge, exactly as before."
    RUBRIC_BLOCK_AGENT="${_rubric_header}For EACH criterion above (in this order: ${_rubric_ids}), include one line, in the SAME write as rule 5 and BEFORE the final REVIEW: line, formatted exactly: 'RUBRIC: <id> | PASS|FAIL | <one-line reason>'. These lines inform your reasoning; they do NOT by themselves decide PASS/BLOCK — the final REVIEW: line you write is still the ONLY thing the merge gate reads to decide the merge, exactly as before."
  fi
fi
...
# ── HERD-230: pin the reviewer's DIFF INPUT to the dispatch sha ─────────────────────────────────
# The watcher keys the verdict to the dispatch sha (HERD_REVIEW_SHA). Live `gh pr diff <PR>` would
# follow a mid-review builder push and desync reviewed-content from the verdict-sha (incident 8b).
# When the pin objects resolve under $MAIN, the reviewer is told to run
#   git -C $MAIN diff <merge-base>..<dispatch-sha>
# from the main checkout. Fail-soft: if the sha/objects cannot be resolved, fall back to today's
# live `gh pr diff` path and journal a soft note (the audit convention). Prompt-cache stability:
# the STABLE preamble below keeps a fixed placeholder (`git diff <merge-base>..<dispatch-sha>`);
# only the trailing THIS REVIEW: line carries the concrete command (and thus the unique sha).
_REVIEW_SHA="${HERD_REVIEW_SHA:-}"
_PIN_MODE="${HERD_REVIEW_PIN_MODE:-}"
if [ -z "$_REVIEW_SHA" ] && [ -n "${PR:-}" ]; then
  # Standalone (no watcher): pin once at start to the current PR head so a mid-review push still
  # cannot desync this run. Best-effort; failure falls through to live-diff below.
  _pin_ref="refs/herd-review/pin-pr-${PR}"
  if git -C "$MAIN" fetch -q "${HERD_REMOTE:-origin}" "+pull/${PR}/head:${_pin_ref}" 2>/dev/null; then
    _REVIEW_SHA="$(git -C "$MAIN" rev-parse --verify "${_pin_ref}^{commit}" 2>/dev/null | tr -d '[:space:]')"
    [ -n "$_REVIEW_SHA" ] && _PIN_MODE="pinned"
  fi
  if [ -z "$_REVIEW_SHA" ]; then
    _REVIEW_SHA="$(gh pr view "$PR" --json headRefOid -q .headRefOid 2>/dev/null | tr -d '[:space:]')"
    [ -n "$_REVIEW_SHA" ] && _PIN_MODE="${_PIN_MODE:-unpinned}"
  fi
fi
_pr_diff_cmd=""
if [ -n "$_REVIEW_SHA" ]; then
  # Prefer an explicit merge-base..sha range (two-dot) so the reviewed content is exactly the
  # commits reachable from the pin and not from DEFAULT_BRANCH — independent of the live PR head.
  _mb="$(git -C "$MAIN" merge-base "${DEFAULT_BRANCH}" "$_REVIEW_SHA" 2>/dev/null | tr -d '[:space:]')"
  _main_q="$(printf '%q' "$MAIN")"
  if [ -n "$_mb" ] && git -C "$MAIN" cat-file -e "${_REVIEW_SHA}^{commit}" 2>/dev/null; then
    _pr_diff_cmd="git -C ${_main_q} diff ${_mb}..${_REVIEW_SHA}"
  elif git -C "$MAIN" cat-file -e "${_REVIEW_SHA}^{commit}" 2>/dev/null; then
    _pr_diff_cmd="git -C ${_main_q} diff ${DEFAULT_BRANCH}...${_REVIEW_SHA}"
  fi
fi
if [ -z "$_pr_diff_cmd" ]; then
  # Fail-soft fallback (HERD-230 audit convention): live PR diff when the pin cannot be materialised.
  _pr_diff_cmd="gh pr diff ${PR}"
  if [ -n "${PR:-}" ]; then
    journal_append review_pin_soft pr "$PR" sha "${_REVIEW_SHA:-}" \
      reason "pin objects unavailable; live-diff fallback" pin_mode "${_PIN_MODE:-}" 2>/dev/null || true
  fi
fi
...
TASK="You are an ADVERSARIAL PRE-MERGE CORRECTNESS REVIEWER for the project '${WORKSPACE_NAME}', where a SILENTLY WRONG result is the worst outcome (it doesn't crash, it just produces bad output/data). Your ONLY job: read the diff of the pull request under review (with 'git diff <merge-base>..<dispatch-sha>' from the main checkout — NOT live 'gh pr diff') and hunt HARD for a concrete CORRECTNESS or DATA-INTEGRITY bug introduced by the diff. Look especially for: ${CHECKLIST_TEXT}. RULES: (1) BLOCK ON CORRECTNESS ONLY. Classify EVERY finding as either CORRECTNESS (a real bug that makes the code produce wrong output/data, crash, corrupt state, or violate an invariant — BLOCKING) or ADVISORY (style, naming, formatting, test coverage, hardening, defensiveness, or subjective design — NON-BLOCKING). ONLY a correctness finding may block the merge; advisory findings are surfaced as non-blocking notes and must NEVER gate the merge. (2) You are READ-ONLY: DO NOT edit any file, DO NOT commit, push, or merge. The only write you may do is ONE 'gh pr comment <PR> --body \"…\"'. (3) DEFAULT TO BLOCK WHEN UNCERTAIN: if you find a real correctness/data-integrity bug, OR you cannot convince yourself the diff is correct, BLOCK. Only PASS when you are confident the diff is correct — a purely advisory finding is NOT a reason to block. (4) Post a brief PR comment summarizing your findings via 'gh pr comment <PR> --body \"…\"' (one tight paragraph: the blocking bug + why it's wrong, or the PASS rationale; then list any ADVISORY style/hardening findings separately as clearly-labelled NON-BLOCKING notes). (5) FINALLY, as the LAST thing you print, output EXACTLY ONE verdict line and nothing after it. BLOCK if and only if you have at least one CORRECTNESS finding — a STRUCTURED verdict of exactly this shape: 'REVIEW: BLOCK — rule: <which correctness rule was violated> | why: <the reasoning> | location: <file:line or function>' (those three ' | '-separated fields, rule/why/location). Otherwise PASS: print exactly 'REVIEW: PASS' when you found NO issues at all, or, when you found ONLY advisory (non-correctness) issues, 'REVIEW: PASS — advisory: <one-line note> | advisory: <one-line note>' with one ' advisory:' segment per advisory finding. Keep it to that single line; it is parsed by a machine; do not add markdown, quotes, or extra text around it.${RUBRIC_BLOCK:+ $RUBRIC_BLOCK} THIS REVIEW: the pull request under review is PR #${PR} (branch slug '${SLUG}'); read its pinned dispatch-sha diff with '${_pr_diff_cmd}' (from the main checkout; do NOT use live 'gh pr diff' — a mid-review push must not change what you review) and post your one comment with 'gh pr comment ${PR} --body \"…\"'."
...
# RUBRIC_FILE (HERD-400): pull this run's 'RUBRIC: <id> | PASS|FAIL | <reason>' lines from wherever
# the reviewer's full output landed — $_agent_result_file (or its near-miss) in agent-pane mode, since
# the agent wrote them there itself in the SAME write as the verdict; $LOG otherwise (headless single-
# reviewer AND the panel branch above, which already cat's every panelist's file into $LOG for
# forensics). Empty whenever RUBRIC_FILE is unset — no rubric block was ever in the prompt, so there
# is nothing to find, and the result file below stays single-line, byte-identical to today.
_RUBRIC_LINES=""
if [ -n "$_rubric_path" ]; then
  if [ "$_AGENT_PANE_MODE" = "1" ]; then
    _RUBRIC_LINES="$(grep -E '^[[:space:]]*RUBRIC: ' "${_nearmiss_path:-$_agent_result_file}" 2>/dev/null || true)"
  else
...
case "$verdict_line" in
  ...
  "REVIEW: PASS"|"REVIEW: PASS "*)
    ...
    _emit_verdict "$verdict_line" "$_RUBRIC_LINES"
    exit 0
    ;;
  "REVIEW: BLOCK"*)
