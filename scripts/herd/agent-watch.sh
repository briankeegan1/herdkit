...
_review_gate_step() {
  ...
  if [ -f "$result" ]; then
    ...
    case "$verdict_line" in
      ...
      # A real verdict (PASS or BLOCK) proves the env is ALIVE → reset the infra breaker's counter.
      "REVIEW: PASS")   _breaker_record_ok; record_review "$pr" "$sha" "PASS"  "reviewer"; echo PASS;  return 0 ;;
      "REVIEW: BLOCK"*)
        _breaker_record_ok
