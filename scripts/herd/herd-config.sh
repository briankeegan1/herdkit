...
: "${GATE_STATUS:="on"}"         # HERD-194: post a `herd/gates` COMMIT STATUS as the watcher clears each (pr,sha) — on (default) | off. on → the watcher posts state=pending at gate dispatch, state=failure on a reproduced healthcheck CODEERROR or a review BLOCK, and state=success on both-gates-green (exactly once per (pr,sha,conclusion), sha-keyed ledger). Pair it with `require herd/gates` GitHub branch protection (recipe: docs/governance-gates.md) so the gate is FAIL-SAFE across seats/collaborators: anyone may merge, but nothing UNGATED can — a commit no watcher blessed has no success status and is unmergeable. In team mode (WATCHER_SCOPE=all) a sha another seat already blessed is skipped (cross-seat dedup). off → byte-inert: no status posted, no read. Consumed by agent-watch.sh
