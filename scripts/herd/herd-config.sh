...
: "${GATE_STATUS:="on"}"         # HERD-194: post a `herd/gates` COMMIT STATUS as the watcher clears each (pr,sha) — on (default) | off. on → the watcher posts state=failure on a reproduced healthcheck CODEERROR or a review BLOCK, and state=success on both-gates-green (exactly once per (pr,sha,conclusion), sha-keyed ledger). It does NOT post a `pending` status — a pending non-passing check would flip a CLEAN sha to UNSTABLE and strand it; the fail-safe needs only the ABSENCE of success. Pair it with `require herd/gates` GitHub branch protection (recipe: docs/governance-gates.md) so the gate is FAIL-SAFE across seats/collaborators: anyone may merge, but nothing UNGATED can — a commit no watcher blessed has no success status and is unmergeable (a fresh PR reports BLOCKED until blessed, which the watcher gates specially so it never deadlocks). In team mode (WATCHER_SCOPE=all) a sha another seat already blessed is skipped (cross-seat dedup). off → byte-inert: no status posted, no read. Consumed by agent-watch.sh
