...
: "${MAINTENANCE_GARDENER:="off"}"  # HERD-673: the MAINTENANCE GARDENER — periodic doc-drift sweep — on | off (default off, ship-dormant). on → `gardener.sh run` reads merge events journaled since its own stored cursor (byte offset in the worktree pool, first run pins at EOF rather than replaying history), diffs each merge's changed files against a small SURFACE set (bin/herd, scripts/herd/*.sh, templates/capabilities.tsv, templates/conformance.tsv) and a DOC set (README.md, docs/**, templates/*.tmpl); a merge that touches SURFACE but no DOC file is real drift. Findings are grouped by file (so two merges touching the same undocumented file in one run collapse into ONE tracker item quoting every merged PR#), filed via scribe.sh, and gated by a PER-SIGNATURE FILING COOLDOWN (a stamp file under the worktree pool; HERD_GARDENER_COOLDOWN_SECS overrides the default 7d window) so a standing drift is not re-filed every run. `--dry-run` prints what would file without calling scribe.sh, stamping cooldown, or advancing the cursor. Zero findings still advances the cursor and journals one `gardener_run findings=0` event — never silence with no trace. FILES, NEVER EDITS: gardener.sh never writes README.md/docs/**/templates/*.tmpl itself, only requests a human/builder look. off (default) → byte-identical: `gardener.sh run` prints a one-line disabled notice and returns 0 — no journal read, no state dir, no scribe call. Consumed by scripts/herd/gardener.sh, scheduled via .herd/triggers.tsv (HERD-169).
