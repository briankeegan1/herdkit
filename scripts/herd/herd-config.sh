...
: "${MAINTENANCE_GARDENER:="off"}"  # HERD-673, tuned HERD-730: the MAINTENANCE GARDENER — periodic doc-drift sweep — on | off (default off, ship-dormant). on → `gardener.sh run` reads merge events journaled since its own stored cursor (byte offset in the worktree pool; an absent, corrupt, or rotated/oversized cursor always re-baselines at the CURRENT journal EOF, never replaying history as archaeology), diffs each merge's changed files against a small SURFACE set (bin/herd, scripts/herd/*.sh, templates/capabilities.tsv, templates/conformance.tsv) and a DOC set (README.md, docs/** except the GENERATED docs/codemap.md and docs/symbol-index.md, templates/*.tmpl, templates/capabilities.tsv itself); a merge that touches SURFACE but no DOC file is real drift, unless the surface file's own diff touches only its leading `#`-comment header block (self-documenting). Every drifted file found in a run is grouped by subsystem (its own directory) into ONE rollup tracker item per run — never one item per file — filed via scribe.sh, gated by a PER-SIGNATURE FILING COOLDOWN (a stamp file under the worktree pool; HERD_GARDENER_COOLDOWN_SECS overrides the default 7d window) so a standing drift is not re-included every run, and bounded by a PER-RUN FILING CAP (HERD_GARDENER_MAX_FINDINGS, default 20) that truncates LOUDLY in the rollup body and holds the cursor back so the same window is honestly rescanned next run. `--dry-run` prints what would file without calling scribe.sh, stamping cooldown, or advancing the cursor. Zero findings still advances the cursor and journals one `gardener_run findings=0` event — never silence with no trace. FILES, NEVER EDITS: gardener.sh never writes README.md/docs/**/templates/*.tmpl itself, only requests a human/builder look. off (default) → byte-identical: `gardener.sh run` prints a one-line disabled notice and returns 0 — no journal read, no state dir, no scribe call. Consumed by scripts/herd/gardener.sh, scheduled via .herd/triggers.tsv (HERD-169).
...
: "${ANOMALY_WAKE_SETTLE_SECS:="300"}"  # HERD-750: seconds AFTER the machine's most recent wake during which a phase measurement is dropped even if its OWN interval never touches the wake boundary — default 300 (5 min); 0 = off. GROUNDED 2026-08-17: HERD-512(a)'s _interval_spans_wake only catches a phase whose OWN [start,end] window contains the sleep/wake edge; several short, clustered tick_cadence/render_pass anomalies (82-285s) filed in the hour after a real 58-minute suspend, because each one started measuring AFTER the machine had already resumed — the system was demonstrably still catching up (cold disk cache, mds/Spotlight reindexing, thermal-throttle recovery) but no boundary fell inside any single one of their own intervals, so the existing guard never fired. This key adds the complementary check: a phase that STARTS within the settle window after the last wake is dropped too — same fail-soft contract as ANOMALY_FILE_COOLDOWN_SECS (no probe, or 0, reads as "no", never suppresses a genuine anomaly), same non-numeric-falls-to-default discipline. Only consulted while ANOMALY_BASELINES is on. Consumed by agent-watch.sh
: "${ANOMALY_THRESHOLD_PCT:="150"}"  # HERD-807: the PERCENT of a phase's own LEARNED p95 a completed instance must EXCEED to count as anomalous — default 150 (1.5x p95). Composes with ANOMALY_FLOOR_MARGIN_SECS: the class-default filing bar is max(ANOMALY_THRESHOLD_PCT% of p95, p95 + ANOMALY_FLOOR_MARGIN_SECS). Only consulted while ANOMALY_BASELINES is on; UNSET is byte-identical to the pre-HERD-807 hardcoded 150. Was an inline engine constant (_ANOMALY_THRESHOLD_PCT) — promoted to a validated key so an operator whose render/review phases carry a fatter tail can widen the bar (raise it) without editing the engine, or tighten it (lower it) on a phase that should alarm sooner; a value at/under 100 files on any reading past the floor. A non-numeric value falls back to the default (herd_numeric warns once). Consumed by agent-watch.sh
: "${ANOMALY_FLOOR_MARGIN_SECS:="30"}"  # HERD-807 (HERD-645's floor, promoted): the ABSOLUTE seconds added to p95 for the anomaly filing floor — default 30. The class-default bar is never lower than p95 + this many seconds, so a phase whose learned p95 sits within ordinary jitter of its nominal period (e.g. tick_cadence's ~47-53s against a 60s tick) does not alarm on jitter even when the pct multiplier alone would. Only consulted while ANOMALY_BASELINES is on; UNSET is byte-identical to the pre-HERD-807 hardcoded 30. Raise it to demand a bigger absolute overrun before filing on a phase with a naturally wide spread; a non-numeric value falls back to the default (herd_numeric warns once). Consumed by agent-watch.sh
...
: "${PR_FETCH_UNIFY:="off"}"     # HERD-675: unify the per-tick PR-list fetch — on | off (default off, ship-dormant). off → byte-identical: the bash render leg's `_prs_fetch_tick` (`gh pr list --json ...`) and the Python engine core's own `discover_via_graphql` (`gh api graphql`) keep fetching independently every due tick exactly as before this key existed — HERD-671 pooled and throttled both but deliberately left the two transports separate, and this is that unification. on → `discover_via_graphql` (the richer schema — it already carries body/baseRefName/reviewDecision/isDraft in one round-trip, now title/mergeable too) becomes the FETCH OWNER: every successful graphql discovery writes its raw roster to a tick-scoped, seat-stamped cache file under the worktree pool (`$TREES/.pr-list-cache-<seat>.json`), and `_prs_fetch_tick` reads a projected view of it — skipping its own `gh pr list` round-trip — WHENEVER that cache is still fresh (bounded to roughly one due-tick's width, since bash's leg runs BEFORE python's within a single watcher tick — see `_watcher_remote_tick_due` — so it can only ever consume the PREVIOUS tick's cache, never this one's). A missing/stale/malformed cache (first tick after a restart, a seat whose pool write failed, PR_FETCH_UNIFY just flipped on) is FAIL-SOFT: `_prs_fetch_tick` falls straight back to its own live `gh pr list` fetch, byte-identical to the off behavior for that one tick. Cross-seat safe: the cache filename is stamped with `herd_engine_seat_id`, so two watchers sharing one worktree pool never read or clobber each other's roster. Consumed by scripts/herd/work-units/git-pr.sh + pysrc/herd/live_runtime.py
...
: "${GROK_WAKE_PROOF:="off"}"    # HERD-803: grok bounce-consumption proof + wake-proof + terminal-state retirement — off (default) | on. Injects --session-id <uuid> into interactive grok builder spawns so _herd_grok_session_lifecycle can track turn state via grok-native surfaces (~/.grok/active_sessions.json for terminal detection; session_search.sqlite updated_at for new-turn detection). off (default) → byte-identical: no session-id injected, no SQLite probe, _waw_progressed falls back to raw cksum-delta for grok the same as any unstructured driver. on → at spawn time a UUID is generated and written to <agent-dir>/grok-session-id and passed as --session-id to grok; before each bounce prompt _herd_grok_updated_at_baseline_write snapshots updated_at; _herd_grok_session_lifecycle returns "idle" if the session is no longer in active_sessions.json (terminal), "working" if updated_at advanced past the baseline (bounce consumed), or "idle" if not yet advanced; _reap_agent_working and _waw_progressed consult this structured source instead of the stale herdr working status / pane cksum. Only takes effect when HERD_DRIVER=grok; all other drivers and the default-off case are byte-identical. FAIL-SOFT: missing sessions file, absent sqlite3, or absent session_id → rc 1 → fallback to raw herdr status / cksum. Consumed by scripts/herd/driver.sh (_herd_herdr_start_agent, _herd_grok_session_id_write, _herd_grok_updated_at_baseline_write, _herd_grok_session_lifecycle, herd_driver_agent_status_resolved_ex) and scripts/herd/agent-watch.sh (_waw_progressed, _handle_block_verdict, _check_fast_bounce, _handle_stale_dup, _handle_health_check, _respawn_builder_in_worktree).
: "${CODEX_STOP_HOOK:="off"}"    # HERD-788: inject Codex lifecycle-hook argv into interactive Codex builder spawns so _reap_agent_working can consult structured turn-state (stop-hook marker) instead of relying on the stale raw herdr working status. off (default) → byte-identical: no hook scripts written, no hook flags added to spawn argv, _reap_agent_working falls back to raw herdr status exactly as before. on → at spawn time a stop-hook script (<agent-dir>/hook/stop.sh) and a user_prompt_submit hook (<agent-dir>/hook/user-prompt-submit.sh) are written and injected via codex's -c flag + --dangerously-bypass-hook-trust; stop fires on turn end (writes last-turn, clears current-turn); user_prompt_submit fires on new turn start (writes current-turn); _herd_codex_hook_lifecycle reads both markers and reports working/idle to _reap_agent_working so a merged Codex pane at idle prompt is reaped even though herdr still reads working. Only takes effect when HERD_DRIVER=codex; all other drivers and the default-off case are byte-identical.
...
export PR_FETCH_UNIFY            # HERD-675: reach the Python engine core the watcher spawns as a child; set as a plain var above, must be exported so live_runtime's discover_via_graphql (`_pr_fetch_unify_enabled`) resolves the SAME on/off as the bash render leg's own `_pr_fetch_unify_enabled` (git-pr.sh) — otherwise the python child never sees this unexported shell var, never writes the unified cache, and the bash leg's own read side (gated on the identical key) always falls back to its live fetch: silently byte-identical to off, never a fault, but the lever would do nothing
