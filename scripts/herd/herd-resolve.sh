...
created=$(herdr tab create --cwd "$DIR" --label "resolve·$SLUG" --no-focus)
...
herdr agent start "resolve·$SLUG" --cwd "$DIR" --tab "$TAB" --split right --no-focus -- claude $CLAUDE_FLAGS "$TASK"
