...
sys.exit(0 if any(
  ...
  # research-step.sh on every drain step) goes stale. If it is stale beyond DRAINER_HEARTBEAT_TIMEOUT,
  ...
  if herd_drainer_hung "$HEARTBEAT" "$DRAINER_HEARTBEAT_TIMEOUT"; then
    echo "⚠️  researcher drainer appears HUNG (no heartbeat for >${DRAINER_HEARTBEAT_TIMEOUT}s) — reclaiming the singleton and spawning a fresh drainer (per-request atomic claim prevents double-draining)."
  else
