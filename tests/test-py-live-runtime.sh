...
# ── (5) HERD-345 regression: WORKTREES_DIR set but NOT exported must not block herd_engine_live_tick ─
...
rc_unset=$?
set -e
[ "$rc_unset" -ne 0 ] || fail "(5a) live tick with WORKTREES_DIR unset must refuse (non-zero) — invariant broken"
...
rc_dry=$?
set -e
[ "$rc_dry" -eq 0 ] || fail "(5b) dry-run tick with WORKTREES_DIR env-passed must succeed (exit 0); got rc=$rc_dry"
...
rc_fault=$?
set -e
[ "$rc_fault" -ne 0 ] || fail "(5c) a bogus-PYTHONPATH tick must fault (non-zero); got rc=0 — fix broke the contract"
[ "$rc_fault" -ne 2 ] || fail "(5c) _HERD_ENGINE_TICK_LAST_ERR was empty after fault — stderr no longer captured"
pass

echo "ALL PASS ($PASS/7 live-runtime checks: unit invariants, full dry-run loop, dispatch-and-wait + scope, journal fail-loud, sole-engine resolution, fault→non-zero, HERD-345 env-contract regression)"
