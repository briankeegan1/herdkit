...
echo base > "$MAINDIR/f.txt"
# HERD-656 fixture: a build-cache file force-committed BEFORE the project ever gitignored its own
# pattern (the classic Godot .uid/.godot mistake) — every worktree cut from `main` inherits both the
# tracked blob AND the gitignore rule that (uselessly, per plain git semantics) already covers it.
printf '*.uid\n.claude/settings.json\n' > "$MAINDIR/.gitignore"
echo 'committed before gitignore took effect' > "$MAINDIR/cache.uid"
git -C "$MAINDIR" add -A -f; git -C "$MAINDIR" commit -qm base
...
for s in merged-clean merged-regen merged-dirty merged-lane closed-unique open-live stale-sha; do mk_wt "$s"; done
...
# HERD-656 / GH #762: ONLY lane-generated + gitignored leftovers — a fresh spawn reproduces every one
# of these, so nothing user-authored is at risk. `.claude/settings.json` and `.herd-limit-sentinel` are
# exact LANE paths the spawn/rate-limit-hook lanes write straight into the worktree; `cache.uid` is the
# tracked-but-now-gitignored build cache from the base commit above, regenerated by "the build".
mkdir -p "$TREESDIR/merged-lane/.claude"
echo '{}' > "$TREESDIR/merged-lane/.claude/settings.json"
echo 'sentinel' > "$TREESDIR/merged-lane/.herd-limit-sentinel"
echo 'regenerated by the build' > "$TREESDIR/merged-lane/cache.uid"
...
if [ "\${1:-}" = "pr" ] && [ "\${2:-}" = "view" ]; then
  case "\${3:-}" in
    ...
    feat/merged-lane) printf 'MERGED\t%s\t107\n' "\$(git -C "$TREESDIR/merged-lane" rev-parse HEAD)" ;;
...
# $HERE (reassigned by sourcing agent-watch.sh, above) is scripts/herd — the real install ROOT is
# TWO levels up from there, same as PYSRC's derivation earlier in this file. A single dirname() here
# would repeat the exact bug this test exists to catch (it did, on the first draft of this test: a
# too-shallow reference silently agreed with a too-shallow implementation).
ENGINE_HOME="$(dirname "$(dirname "$HERE")")"
# _sweep_engine_home's TWO branches (HERDKIT_HOME exported vs. derived from BASH_SOURCE) must AGREE on
# the SAME real install — this is exactly the class of bug review caught on the first version of this
# fix (#822): the derived branch silently returned .../scripts instead of the true root, which the
# behavioral check below alone did not catch (the wrong-but-nested value still happened to sit above
# healthcheck.sh's own path). Assert the VALUE, not just downstream behavior on one path.
_prior_hkh="${HERDKIT_HOME:-}"
unset HERDKIT_HOME
GOT_HOME_DERIVED="$(_sweep_engine_home)"
[ "$GOT_HOME_DERIVED" = "$ENGINE_HOME" ] \
  || fail "(23) _sweep_engine_home (derived) = '$GOT_HOME_DERIVED', want '$ENGINE_HOME'"
export HERDKIT_HOME="/some/other/install"
GOT_HOME_EXPORTED="$(_sweep_engine_home)"
[ "$GOT_HOME_EXPORTED" = "/some/other/install" ] \
  || fail "(23) _sweep_engine_home (exported) = '$GOT_HOME_EXPORTED', want '/some/other/install'"
if [ -n "$_prior_hkh" ]; then export HERDKIT_HOME="$_prior_hkh"; else unset HERDKIT_HOME; fi
ok; echo "PASS (23) _sweep_engine_home: derived and exported branches agree on the real install root"
...
grep -q "reap worktree merged-lane" <<< "$PLAN" \
  || fail "(3) HERD-656: plan omits the reap of a worktree holding only lane-generated + gitignored leftovers"
...
[ ! -d "$TREESDIR/merged-lane" ] \
  || fail "(4) HERD-656 / GH #762: a merged worktree holding only .claude/settings.json, .herd-limit-sentinel, and a tracked-but-gitignored build cache was refused instead of reaped"
[ -d "$TREESDIR/merged-dirty" ] \
  || fail "(4) HERD-656: a merged worktree carrying a GENUINE tracked edit (f.txt, no lane/gitignore match) must still be refused, not reaped"
