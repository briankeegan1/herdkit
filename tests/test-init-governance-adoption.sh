...
grep -q "Governance adoption" <<< "$out" || fail "(1) governance adoption should run when CLAUDE.md is present"
...
grep -q 'evidence: "Never auto-merge pull requests without approval"' <<< "$out" || fail "(1) proposal must quote its source sentence as evidence"
...
grep -q "Recognized hook rules" <<< "$out" || fail "(1) hook rule must be surfaced as recognized/deferred"
grep -q "hook: pre-action:run-checks" <<< "$out" || fail "(1) hook surface target must be shown"
...
grep -q "Gap report" <<< "$out" || fail "(1) gap report must be printed"
grep -qF "This project targets Python 3.11" <<< "$out" || fail "(1) unmapped sentence must appear in the gap report"
ok
...
cfg="$proj/.herd/config"
grep -q "Governance adoption" <<< "$out" && fail "(2) no CLAUDE.md → the governance pass must print nothing"
...
cfg="$proj/.herd/config"
grep -q "Governance adoption" <<< "$out" || fail "(3) governance adoption should still run (offer accepted)"
...
grep -q "governance adoption: nothing applied" <<< "$out" || fail "(3) decline-all → summary must report nothing applied"
ok
