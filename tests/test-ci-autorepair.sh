...
grep -q 'ci-repair' <<< "$(row)" || fail "(2) row must name ci-repair (got: $(row))"
grep -q 'awaiting push' <<< "$(row)" || fail "(2) row must read awaiting push (got: $(row))"
...
grep -q 'ci-repair' <<< "$(row)" || fail "(5) once-guard row must still say ci-repair"
...
grep -q 'awaiting push' <<< "$(row)" || fail "(6) resolver path must read awaiting push (got: $(row))"
