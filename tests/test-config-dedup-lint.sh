...
grep -qx "FOO" <<< "$dupes" || fail "1a: scanner missed dup FOO ($dupes)"
grep -qx "BAR" <<< "$dupes" || fail "1b: scanner missed dup BAR (export+plain forms) ($dupes)"
grep -qx "INTERACTION_TEST_CMD" <<< "$dupes" || fail "1c: scanner missed dup INTERACTION_TEST_CMD ($dupes)"
grep -qx "BAZ" <<< "$dupes" && fail "1d: scanner wrongly reported single-assignment BAZ"
...
grep -qxF "FOO=2" <<< "$vals" || fail "2a: last-wins changed — FOO should resolve to 2 ($vals)"
grep -qxF "ITC=[]" <<< "$vals" || fail "2b: last-wins changed — empty INTERACTION_TEST_CMD should win ($vals)"
okc
...
grep -qi 'duplicate key' <<< "$docout" || fail "3b: doctor did not flag the dup"
grep -q 'INTERACTION_TEST_CMD' <<< "$docout" || fail "3c: doctor did not name the duplicated key"
okc
...
grep -q 'no duplicate keys' <<< "$docclean" || fail "3d: doctor did not report a clean config as OK"
grep -qi 'duplicate key(s)' <<< "$docclean" && fail "3e: doctor flagged a dup on a clean config"
okc
...
grep -q 'INTERACTION_TEST_CMD' <<< "$lintout" || fail "4b: 'herd config lint' did not list the dup key ($lintout)"
grep -qi 'last-wins' <<< "$lintout" || fail "4c: 'herd config lint' did not explain last-wins"
okc
...
grep -qi 'no duplicate keys' <<< "$cleanout" || fail "4e: 'herd config lint' clean message missing ($cleanout)"
okc
