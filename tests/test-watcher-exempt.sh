...
#       all children of the canonical watcher). sweep.sh's sweep_stray_watchers ALREADY had the right
#       extra exemption. Both seats now call watcher_list_mains — one check, one answer.
...
#   (d) marker-owned gate worker / a fork parenting a live gate worker → exempt (HERD-217/237/245)
#   (e) bin/herd's _list_project_watchers and sweep.sh's sweep_stray_watchers AGREE on the same table
...
FORK=800002      # a transient tick fork: ppid == canonical            → exempt (child guard)
GATEFORK=800003  # a fork reparented to init, still parenting a gate worker → exempt (child-of guard)
HCPID=800004     # GATEFORK's live healthcheck worker (untagged)
MARKED=800005    # a marker-owned gate worker, reparented to init      → exempt (marker guard)
...
# ── (d) gate-worker exemptions (HERD-217 / HERD-237 / HERD-245) ─────────────────────────────────
printf '%s\n' "$MAINS" | grep -qx "$GATEFORK" \
  && fail "(d) a reparented fork parenting a live healthcheck was listed — the kill path would sever it"
printf '%s\n' "$MAINS" | grep -qx "$MARKED_REAL" \
  && fail "(d) a LIVE inflight-marker worker was listed as a duplicate watcher"
...
ok; echo "PASS (d) marker-owned + gate-child forks are exempt; a foreign workspace is never listed"
...
[ "$(printf '%s\n' "$MAINS" | grep -c .)" -eq 2 ] \
  || fail "(b) expected exactly 2 mains (canonical + orphan), got: '$(printf '%s' "$MAINS" | tr '\n' ' ')'"
# … and the status alarm VERIFIES it as real (every re-sample still sees both).
HERD_STATUS_DUP_SAMPLES=3 HERD_STATUS_DUP_SLEEP=0 _status_dup_verified "$MAINS" \
  || fail "(b) a persistent genuine duplicate did not verify — the alarm would never fire"
ok; echo "PASS (b) a genuine orphan duplicate is listed and the alarm verifies it as real"
...
# ── (e) both seats agree on the same table ─────────────────────────────────────────────────────
# sweep_stray_watchers is watcher_list_mains minus the lockfile pid. Load sweep.sh the way the CLI
# does (agent-watch.sh in LIB mode), then drive it against the SAME planted table. Sourcing needs a
# project shape, so give it one; only the pure function is exercised.
...
if [ -n "$STRAY" ]; then
  ...
  ok; echo "PASS (e) sweep_stray_watchers and the shared check agree on the same process table"
else
  # sweep.sh could not be loaded in this environment (it pulls in agent-watch.sh's full substrate).
  # The agreement is then proven structurally: sweep_stray_watchers is a thin filter over the shared
  # listing, so it cannot disagree by construction. Assert that structure instead of skipping silently.
  ...
  ok; echo "PASS (e) sweep_stray_watchers is a thin filter over the shared watcher_list_mains (structural)"
fi
