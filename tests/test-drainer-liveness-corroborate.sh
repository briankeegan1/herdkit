...
grep -q "drainer dispatched" <<< "$out" || fail "(3) with no drainer listed, scribe must spawn one (got: $out)"
...
grep -qi "reclaiming the singleton\|is DEAD" <<< "$out" && fail "(4a) a live drainer must NOT be declared dead/reclaimed (got: $out)"
...
grep -qi "DEAD\|reclaiming" <<< "$out" || fail "(4b) a genuinely dead drainer must be reclaimed (got: $out)"
...
grep -qi "agent_name_taken\|error:" <<< "$out" && fail "(4c) a failed respawn must NOT dump the raw driver error (got: $out)"
grep -qi "already holds the name\|queued" <<< "$out" || fail "(4c) a failed respawn must print a clean, reassuring message (got: $out)"
